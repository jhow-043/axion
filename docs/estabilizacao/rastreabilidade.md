...
| EST-FE-003 | P02 | 10 | Alto | Lint frontend nao executa por falta de `eslint.config.*` compativel com ESLint 9 | #35 | - | - | Issue aberta |
| EST-DEV-001 | DX | Geral | Alto | `start.ps1` imprime erros `PID : O termo 'PID' nao e reconhecido` ao iniciar Celery/Vite | #30 | fix/EST-DEV-001-start-pid-output | #31 | Mergeado |
| EST-FE-004 | P02 | 10 | Medio | React Router emite warning `No HydrateFallback element provided` no console do navegador | #32 | - | - | Issue aberta |
| EST-AUTH-001 | P03 | 01 | Medio | Console mostra varios 401 em `/auth/refresh` e `/auth/login` ao abrir/login; precisa separar fluxo esperado de bug real | #33 | - | - | Issue aberta |
| EST-BE-001 | P00-P19 | Geral | Alto | `ruff check .` falha com 59 ocorrencias em codigo e testes | #36 | - | - | Issue aberta |
| EST-BE-002 | P00 | Geral | Medio | `Settings.DEBUG` quebra testes quando ambiente externo define `DEBUG=release` | #37 | - | - | Issue aberta |
| — | — | — | — | *Aguardando Fase A* | — | — | — | — |
