...
import { useDeactivateLocation, useLocations, useReactivateLocation } from "../api";
...
export function LocationList() {
  ...
  const { data, isLoading, error } = useLocations(filters);
  const deactivate = useDeactivateLocation();
  const reactivate = useReactivateLocation();
  ...
  if (error) {
    return <div className="p-6 text-red-600">Erro ao carregar locais. Tente novamente.</div>;
  }

  if (creating) {
    return (
      <div className="p-6 max-w-lg">
        <h2 className="text-xl font-semibold mb-4">Novo Local</h2>
        <LocationForm onSuccess={() => setCreating(false)} onCancel={() => setCreating(false)} />
      </div>
    );
  }
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold">Locais Prediais</h1>
        {canManage && (
          <button
                ...
                <tr key={loc.id} className="hover:bg-gray-50">
                  ...
                  {canManage && (
                    <td className="px-4 py-3 space-x-3">
                      ...
                      {loc.is_active ? (
                        confirmDeactivate === loc.id ? (
                          <span className="inline-flex gap-1 text-xs">
                            <button
                              onClick={() => {
                                deactivate.mutate(loc.id);
                          ...
                          <button
                            onClick={() => setConfirmDeactivate(loc.id)}
                        ...
                        <button
                          onClick={() => reactivate.mutate(loc.id)}
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between text-sm text-gray-600">
          <span>
            {data?.total} locais — página {filters.page} de {totalPages}
          </span>
