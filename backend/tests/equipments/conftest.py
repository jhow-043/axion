...
async def tenant(db_session: AsyncSession) -> Tenant:
    t = Tenant(name="Equipment Test Corp", slug=f"eq-test-{uuid.uuid4().hex[:8]}")
    db_session.add(t)
...
@pytest.fixture
async def active_sector(db_session: AsyncSession, seeded_tenant: Tenant) -> Sector:
    sector = Sector(
        tenant_id=seeded_tenant.id,
        name=f"Mecânica {uuid.uuid4().hex[:4]}",
        is_active=True,
    )
...
@pytest.fixture
async def inactive_sector(db_session: AsyncSession, seeded_tenant: Tenant) -> Sector:
    sector = Sector(
        tenant_id=seeded_tenant.id,
        name=f"Inativo {uuid.uuid4().hex[:4]}",
        is_active=False,
    )
    db_session.add(sector)
    await db_session.flush()
