...
async def tenant(db_session: AsyncSession) -> Tenant:
    t = Tenant(name="Users Test Corp", slug=f"users-test-{uuid.uuid4().hex[:8]}")
    db_session.add(t)
...
async def seeded_tenant(db_session: AsyncSession, tenant: Tenant) -> Tenant:
    """Tenant with default roles and permissions seeded."""
...
async def _make_bearer(user: User, db_session: AsyncSession) -> str:
    """Creates a valid JWT Bearer token by reading the user's roles from the session."""
    stmt = (
        select(Role.code)
...
def _make_client(db_session: AsyncSession, auth_header: str | None = None) -> AsyncClient:
    """Creates an AsyncClient with the test DB override and optional auth header.
    Must be used as an async context manager by the caller."""
...
async def users_client(db_session: AsyncSession):
    """Unauthenticated HTTP client with test DB override."""
...
async def tech_client(db_session: AsyncSession, technician_user: User):
    """HTTP client authenticated as technician (no user:manage permission)."""
