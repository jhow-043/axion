...
async def tenant(db_session: AsyncSession) -> Tenant:
    t = Tenant(name="Teams Test Corp", slug=f"teams-test-{uuid.uuid4().hex[:8]}")
    db_session.add(t)
...
@pytest.fixture
async def inactive_user(db_session: AsyncSession, seeded_tenant: Tenant) -> User:
    user = User(
        tenant_id=seeded_tenant.id,
        name="Inactive User",
...
@pytest.fixture
async def teams_client(db_session: AsyncSession):
