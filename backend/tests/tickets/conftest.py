...
async def tenant(db_session: AsyncSession) -> Tenant:
    t = Tenant(name="Tickets Test Corp", slug=f"tkt-{uuid.uuid4().hex[:8]}")
    db_session.add(t)
...
async def admin_user(db_session: AsyncSession, seeded_tenant: Tenant, admin_role: Role) -> User:
    user = _make_user(seeded_tenant.id, "Admin User", f"admin-{uuid.uuid4().hex[:6]}@test.com")
    db_session.add(user)
    await db_session.flush()
...
async def technician_user(
    ...
    user = _make_user(seeded_tenant.id, "Tech User", f"tech-{uuid.uuid4().hex[:6]}@test.com")
    db_session.add(user)
    await db_session.flush()
    db_session.add(
...
@pytest.fixture
async def inactive_location(db_session: AsyncSession, seeded_tenant: Tenant) -> Location:
    loc = Location(
        tenant_id=seeded_tenant.id,
        name=f"Sala Inativa {uuid.uuid4().hex[:4]}",
        is_active=False,
    )
    db_session.add(loc)
    await db_session.flush()
...
@pytest.fixture
async def pending_reason(db_session: AsyncSession, seeded_tenant: Tenant) -> PendingReason:
    reason = PendingReason(
        tenant_id=seeded_tenant.id,
        name=f"Aguardando peça {uuid.uuid4().hex[:4]}",
        is_active=True,
    )
